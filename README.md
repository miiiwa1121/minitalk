...
```
- `si_pid` : シグナルを送信したプロセスのPID（プロセスID）
- `si_uid` : シグナルを送信したユーザーのUID（ユーザーID）

`si_pid`は、`siginfo_t`構造体の`_sifields`内の`_kill`サブ構造体に含まれています。`sa_flags`に`SA_SIGINFO`を設定すると、`sigaction`でこの情報を取得できます。
```
- `si_signo` : シグナル番号
```
どのシグナルが発生したかを表します。
...
- #### Q : この条件式は、文字のビットをどのようにチェックしているのですか？
```
if ((c & (1 << bit)) != 0):
```
> A : 例えば1<<3は00000100と同意と考えることができる、c=00011110だったとすると00000100と00011110の論理積で00000100となるため、cのbit目が0ではないとわかる。

### サーバーコード

- #### Q : この行は、受信したシグナルビットから文字をどのように再構成しているのですか？
```
c |= (1 << bit);
```
> A : static int cによってビットを保持し、１バイト分揃った時点で標準出力をしている
...
- #### Q : kill(info->si_pid, signal);の「->」の意味は？
> A : 実際には (*info_ptr).si_pid と同じ意味ですが、コードの可読性を考慮して -> を使います。
		

- #### Q : kill(info->si_pid, signal);のsi_pidの意味は？
> A : si_pid は、siginfo_t 構造体のメンバで、シグナルを送信したプロセスのプロセス ID (PID) を表します。
...
```
> A : exit関数の戻り値で`EXIT_SUCCESS`,`EXIT_FAILURE`があり、それぞれ`0`,`1`が割り当てられている定数である。
...
> A : シグナルの受信をカーネルが管理している仕組みを利用して認知します。
